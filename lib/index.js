...
const packet = JSON.parse(fs.readFileSync(path.join(__dirname, '/../package.json'), 'utf8'))
...
program
  ...
  .option('-q, --query [query]', 'join query or path to (json, or yml) file that contains the query')
  .option('-f, --file [filepath]', 'output file (xls, csv, json)')
  .option('-k, --key [file]', 'absolute path to private ssh key (only for ssh auth)')
  .parse(process.argv)

// Input validaton
if (!program.query) throw console.error('no query specified. always specify query as (json or yaml) file pointer or json string')
...
joinMongoCollections(program, (err, result) => {
  ...
  let format
  if (program.file && program.file.includes('.')) format = program.file.split('.').slice(-1)[0]
