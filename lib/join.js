...
const fetchMongo = (query, projection, db, collection, sortKey, key, callback) => {
  ...
  } else return callback(new Error('unsupported protocol'))
}
...
const fetchMongoOverSsh = (query, projection, url, collection, sortKey, key, callback) => {
  ...
  } else return callback(new Error('no auth method defined (keyfile or password)'))
      ...
      .then(function () {
        ssh.execCommand(mongoCommand)
          .then(function (result) {
            ...
            let resultObj
            try {
              resultObj = JSON.parse(JSON.parse(JSON.stringify(result.stdout)))
            } catch (e) {
              return callback(new Error(e + ' -> ' + result + ' -> ' + mongoCommand))
            }
...
/**
 * Returns a value at the leaf of a object tree with one
 * or more branches
 * 
 * @param {object} obj 
...
const joinArraysOnKey = (array1, key1, array2, key2, options) => {
  ...
  function binarySearch (array, key, searchValue, options) {
    // console.log(array.length, key, searchValue, options)
   ...
   * @param {object} options
   */
  let simpleSearch = (array, key, value, options) => {
    let items = []
    options = options || {}
    ...
    if (options.distinct && items[0]) items = [items[0]]
    return items
  }

  if (!options.distinct && options.method === 'binarySearch') {
    console.error('Error: non distinct binary search is not yet supported')
    throw new Error('non distinct binary search is not yet supported')
  }
  ...
  array1.forEach(item1 => {
    if (resolve(item1, key1)) {
      let joinedItems = []
      if (options.method === 'binarySearch') {
        joinedItems = binarySearch(array2, key2, resolve(item1, key1), options)
      } else {
        joinedItems = simpleSearch(array2, key2, resolve(item1, key1), options)
      }
